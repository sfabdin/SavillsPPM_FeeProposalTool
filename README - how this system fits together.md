...
Ingestion Studio, Revenue Projections, Staffing Matrix, Revenue Reconciliation
and the rest) collect the team's project, fee, staffing, rate and actuals data.
The shared state lives in five Box files the tool maintains: `projects.json`,
`rates.json`, `staff.json`, `studio.json` and `revenue.json` (the monthly-close
ledger and its mapping book).
...
over four of them (`projects.json`, `rates.json`, `staff.json`,
`studio.json`): pipeline against budget, leaders, clients, rates, locations,
delivery and data confidence. It is restricted to the sees-all-projects list
and never writes.
